"""
BaseAgentForex class - Base class for forex trading agents
...
class BaseAgentForex:
    """
    Base class for forex trading agents
    ...
    # Precious metals (require premium Alpha Vantage API)
    PRECIOUS_METALS = ["XAUUSD", "XAGUSD"]

    # Default forex pairs (free tier compatible)
    ...
    def __init__(
        ...
        Args:
            ...
            forex_pairs: List of forex pairs, defaults to majors + precious metals
            mcp_config: MCP tool configuration, including port and URL information
            log_path: Log path, defaults to ./data/agent_data_forex
            max_steps: Maximum reasoning steps
            max_retries: Maximum retry attempts
            base_delay: Base delay time for retries
            openai_base_url: OpenAI API base URL
            openai_api_key: OpenAI API key
            initial_cash: Initial cash amount in USD
            init_date: Initialization date
            market: Market type, hardcoded to "forex"
        """
        self.signature = signature
        self.basemodel = basemodel
        self.market = "forex"  # Hardcoded to forex

        # Auto-select forex pairs if not provided
        if forex_pairs is None:
            self.forex_pairs = self.DEFAULT_FOREX_PAIRS
        else:
    ...
    async def run_trading_session(self, today_date: str) -> None:
        ...
        """
        print(f"📈 Starting forex trading session: {today_date}")
        ...
        self.agent = create_agent(
            self.model,
            tools=self.tools,
            system_prompt=get_agent_system_prompt_forex(today_date, self.signature, self.market, self.forex_pairs),
        )

        # Initial user query
        user_query = [{"role": "user", "content": f"Please analyze and update today's ({today_date}) forex positions."}]
    ...
    def register_agent(self) -> None:
        ...
        init_position = {pair: 0.0 for pair in self.forex_pairs}
        init_position["CASH"] = self.initial_cash

        with open(self.position_file, "w") as f:
            f.write(json.dumps({"date": self.init_date, "id": 0, "positions": init_position}) + "\n")

        print(f"✅ Forex Agent {self.signature} registration completed")
        print(f"📁 Position file: {self.position_file}")
        print(f"💰 Initial cash: USD${self.initial_cash:,.2f}")
        print(f"📊 Number of forex pairs: {len(self.forex_pairs)}")
    ...
    def get_trading_dates(self, init_date: str, end_date: str) -> List[str]:
        ...
        Returns:
            List of trading dates (forex trades 5 days a week, Monday-Friday)
        """
    ...
    async def run_date_range(self, init_date: str, end_date: str) -> None:
        ...
        print(f"✅ {self.signature} forex processing completed")
    ...
    def __str__(self) -> str:
        return (
            f"BaseAgentForex(signature='{self.signature}', basemodel='{self.basemodel}', pairs={len(self.forex_pairs)})"
        )
